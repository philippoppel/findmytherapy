  ...
  try {
    ...
    switch (topic) {
      case 'client-registration': {
        const { email, firstName } = payload as { email: string; firstName?: string; userId: string; marketingOptIn?: boolean }
        if (email && firstName) {
      ...
      case 'therapist-registration': {
        const { email, firstName, city } = payload as { email: string; firstName?: string; city?: string; userId: string }
