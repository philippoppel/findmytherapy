...
import { DEFAULT_WEIGHTS, PROBLEM_AREA_MAPPING, AVAILABILITY_STATUS_WEEKS } from './types';
...
function calculateSpecialtyScore(
  ...
  for (const problemArea of preferences.problemAreas) {
    ...
    for (const specialty of relatedSpecialties) {
      ...
      if (
        therapistSpecialties.some(
          (ts) => ts.includes(normalizedSpecialty) || normalizedSpecialty.includes(ts),
        )
      ) {
        totalMatches++;
      }
...
function calculateMethodsScore(
  ...
  for (const method of preferences.preferredMethods) {
    ...
    if (
      therapistMethods.some((tm) => tm.includes(normalizedMethod) || normalizedMethod.includes(tm))
    ) {
...
function calculateProfileQualityScore(therapist: TherapistForMatching): {
  score: number;
  factors: string[];
} {
  let score = 0.5; // Basis
...
export function calculateMatchScore(
  ...
  const { score: availabilityScore, waitWeeks } = calculateAvailabilityScore(
    preferences,
    therapist,
  );
  ...
  const { score: profileQualityScore, factors: profileFactors } =
    calculateProfileQualityScore(therapist);
