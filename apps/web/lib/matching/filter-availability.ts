  ...
  const baseTherapists = await prisma.therapistProfile.findMany({
    where: {
      ...
      ...(currentFilters.insuranceType &&
        currentFilters.insuranceType !== 'ANY' && {
          OR: [
            // Public insurance
            ...
            ...(currentFilters.insuranceType === 'PRIVATE'
              ? [
                  {
                    AND: [{ privatePractice: true }, { acceptedInsurance: { isEmpty: false } }],
                  },
                ]
              : []),
            // Self-pay
...
function calculateInsuranceOptions(
  ...
  return [
    ...
    { value: 'PRIVATE', label: 'Privat', count: counts.PRIVATE, available: counts.PRIVATE > 0 },
    {
      value: 'SELF_PAY',
      label: 'Selbstzahler',
      count: counts.SELF_PAY,
      available: counts.SELF_PAY > 0,
    },
  ];
}
