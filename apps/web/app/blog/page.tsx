    ...
    <div className="min-h-screen bg-gradient-to-b from-teal-950/5 via-cyan-950/5 to-teal-950/5 py-12 sm:py-16">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 space-y-12 sm:space-y-16">
        ...
        <div>
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-sm font-semibold text-teal-700 transition hover:text-teal-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 focus-visible:ring-offset-2"
          >
        ...
        <header className="text-center space-y-4 sm:space-y-6" aria-labelledby="blog-title">
          <span className="inline-flex items-center rounded-full bg-teal-100 px-4 py-1.5 text-sm font-semibold text-teal-800 ring-1 ring-inset ring-teal-600/20">
            FindMyTherapy Blog
          </span>
        ...
        <section aria-labelledby="featured-title">
          ...
          <article className="group relative overflow-hidden rounded-3xl bg-gradient-to-br from-teal-50 via-cyan-50/50 to-white border border-teal-200/60 shadow-xl shadow-teal-900/10 transition hover:shadow-2xl hover:shadow-teal-900/15">
            <div className="grid grid-cols-1 gap-8 p-8 sm:p-10 md:grid-cols-5">
              <div className="md:col-span-3 space-y-5">
                ...
                <h3 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
                  <Link href={`/blog/${featuredPost.slug}`} className="transition-colors hover:text-teal-700">
                    {featuredPost.title}
                  </Link>
                </h3>
              ...
              <div className="md:col-span-2 space-y-4 rounded-2xl border border-teal-200/60 bg-white/80 p-6 backdrop-blur">
        ...
        <section aria-labelledby="all-posts-title" className="space-y-8">
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {otherPosts.map((post) => (
              <article
                key={post.slug}
                className="group flex h-full flex-col justify-between rounded-2xl border border-gray-200 bg-white p-6 shadow-md shadow-gray-900/5 transition hover:-translate-y-1 hover:border-teal-300 hover:shadow-lg hover:shadow-teal-900/10"
              >
                <div className="space-y-4">
                  <div className="flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-wide text-gray-500">
                    <span className="rounded-full bg-teal-100 px-2 py-0.5 text-teal-800">{post.category}</span>
                  ...
                  <h3 className="text-xl font-bold text-gray-900">
                    <Link href={`/blog/${post.slug}`} className="transition-colors group-hover:text-teal-700">
                      {post.title}
                    </Link>
                  </h3>
