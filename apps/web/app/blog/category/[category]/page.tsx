...
export function generateMetadata({ params }: CategoryPageProps): Metadata {
  ...
  const categoryPosts = blogPosts.filter(
    (post) => slugifySegment(post.category) === params.category,
  );
              ...
              <div className="relative z-10 space-y-6">
                ...
                <div className="space-y-4 text-center sm:text-left">
                  ...
                  <p className="mx-auto max-w-3xl text-lg text-white/90 sm:mx-0">
                    Kategoriespezifische Recherchen, Interviews und Produkt-Einblicke. Wir zeigen
                    den aktuellen Stand unserer Arbeit und welche Fragen wir gemeinsam mit
                    Expert:innen beantworten.
                  </p>
                </div>
                    ...
                    >
                      <div className="flex flex-wrap items-center gap-3 text-xs text-neutral-500">
                        ...
                        {post.tags?.slice(0, 2).map((tag) => (
                          <span
                            key={tag}
                            className="rounded-full bg-primary-50 px-3 py-1 font-semibold text-primary-900"
                          >
                            #{tag}
                          </span>
                        ))}
            ...
            <section aria-labelledby="related-tags" className="space-y-6">
              ...
              <div className="flex flex-wrap gap-3">
                {relatedTags.length === 0 && (
                  <span className="text-sm text-neutral-500">Noch keine Tags vorhanden.</span>
                )}
                {relatedTags.map((tag) => (
                  <Link
                    key={tag}
              ...
              <div className="relative z-10 grid gap-8 lg:grid-cols-2">
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-white/90">
                    Teile deinen Themenwunsch, schick eine Research-Frage oder sichere dir die
                    ersten Updates zur Produkt-Roadmap. Wir antworten persönlich.
                  </p>
