...
import { AlertTriangle, ArrowRight, BookOpen, Brain, CheckCircle2, HeartPulse, RotateCcw, Sparkles } from 'lucide-react'
    ...
    return (
      <div className="rounded-3xl border border-divider bg-white/90 p-8 shadow-lg shadow-primary/10 backdrop-blur">
        <header className="space-y-2">
          ...
          <h3 className="text-2xl font-semibold text-neutral-900">Deine Klarthera Empfehlung</h3>
          <p className="text-sm text-muted">
            Auf Basis deiner Angaben schlagen wir das folgende Paket vor. Die vollständige Demo zeigt, wie Matches und Kurszugänge freigeschaltet werden.
          </p>
        </header>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div className="rounded-2xl border border-divider bg-surface-1/90 p-5">
            ...
            <p className="mt-2 text-lg font-semibold text-neutral-900">{levelLabel}</p>
        ...
        <div className="mt-6 rounded-2xl border border-primary/20 bg-primary/5 p-6 text-sm leading-relaxed text-primary-900">
          {recommendation}
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-[1.2fr_0.8fr]">
          ...
          <div className="flex flex-col justify-between rounded-2xl border border-divider bg-white/85 p-6 shadow-sm shadow-primary/5">
            <div className="space-y-2">
              <p className="text-xs font-semibold uppercase tracking-wide text-primary">Demo-Optionen</p>
              <p className="text-sm text-muted">
                Starte mit einer*m verifizierten Therapeut:in oder aktiviere das passende Kursmodul. Termine sind in der Demo sofort buchbar.
              </p>
            </div>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-between gap-4">
          ...
          <p className="text-xs text-subtle">
            Hinweis: Die Ergebnisse sind Teil der Demo. Im Produkt werden medizinische Schwellenwerte geprüft und Notfallroutinen ausgelöst.
          </p>
        </div>
      </div>
    )
    ...
    >
      <header className="space-y-2">
        <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-primary">
          ...
          Demo-Schritt {stepIndex + 1} von {questions.length}
        </div>
        <h3 className="text-2xl font-semibold text-neutral-900">{currentQuestion.title}</h3>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-3 rounded-2xl border p-4 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-surface ${
                      selected
                        ...
                        : 'border-divider bg-surface-1/95 text-muted hover:border-primary/30 hover:text-primary-900'
                    }`}
                  >
