...
describe('TriageFlow - Security & Crisis Features', () => {
  ...
  beforeEach(() => {
    ...
    fetchMock.mockResolvedValue(
      mockFetchResponse({ success: true, recommendations: mockRecommendations }),
    );
  });
  ...
  describe('PHQ-9 Item 9 - Suizid-Detektion', () => {
    it('should show crisis banner when PHQ-9 Item 9 is answered with ≥1', async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/wichtig: sofortige unterstützung verfügbar/i),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
    ...
    it('should NOT show crisis banner when PHQ-9 Item 9 is answered with 0', async () => {
      ...
      expect(
        screen.queryByText(/wichtig: sofortige unterstützung verfügbar/i),
  ...
  describe('Disclaimer-System', () => {
    ...
    it('should show disclaimer on results page', async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/diese ersteinschätzung ist keine medizinische diagnose/i),
          ).toBeInTheDocument();
        },
        { timeout: 10000 },
      );
  ...
  describe('Navigation', () => {
    it('should allow going back to previous question', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/wenig interesse oder freude an ihren tätigkeiten/i),
        ).toBeInTheDocument();
      });
    });
