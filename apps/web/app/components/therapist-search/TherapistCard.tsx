...
export function TherapistCard({ therapist }: TherapistCardProps) {
  ...
  return (
    <Link href={`/therapists/${therapist.id}`}>
      <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-neutral-200/60 bg-white shadow-sm transition-all duration-300 hover:shadow-lg hover:border-neutral-300">
        {/* Image Section */}
        ...
        <div className="flex flex-1 flex-col gap-4 p-6">
          ...
          {/* CTA */}
          <button className="mt-2 flex items-center justify-center rounded-xl border-2 border-teal-100 bg-teal-50 py-3 text-sm font-semibold text-teal-700 transition-all duration-200 group-hover:border-teal-200 group-hover:bg-teal-100 group-hover:text-teal-800">
            Profil ansehen
          </button>
        </div>
      </article>
