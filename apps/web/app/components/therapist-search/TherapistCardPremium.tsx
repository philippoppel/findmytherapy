...
export function TherapistCardPremium({ therapist }: TherapistCardPremiumProps) {
  ...
  return (
    <Link href={`/therapists/${therapist.id}`}>
      <article className="group relative flex h-full flex-col overflow-hidden rounded-2xl border-2 border-amber-200/80 bg-white shadow-md transition-all duration-300 hover:shadow-xl hover:border-amber-300">
        {/* Subtle Premium Badge */}
        <div className="absolute right-3 top-3 z-20 flex items-center gap-1 rounded-full bg-gradient-to-r from-amber-50 to-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-700 shadow-sm border border-amber-200/50 backdrop-blur-sm">
        ...
        <div className="flex flex-1 flex-col gap-4 p-6 bg-gradient-to-b from-amber-50/30 to-white">
          ...
          {/* CTA */}
          <button className="mt-2 flex items-center justify-center rounded-xl border-2 border-amber-200 bg-amber-50 py-3 text-sm font-semibold text-amber-700 transition-all duration-200 group-hover:border-amber-300 group-hover:bg-amber-100 group-hover:text-amber-800">
            Profil ansehen
          </button>
        </div>
      </article>
