...
const defaultCopy: Record<Variant, { title: string; description: string }> = {
  newsletter: {
    ...
    description: 'Monatliche Insights zu neuen Features, Studien und mental-health-relevanten Ressourcen.',
  },
  topic: {
    title: 'Themenwunsch teilen',
    description: 'Sag uns, welche Fragen wir beantworten dürfen – wir bereiten fundierte Artikel und Interviews vor.',
  },
}
...
export function NewsletterForm({
  ...
  const isDisabled = !form.name || !form.email || form.message.trim().length < 10 || !form.consent || status === 'loading'
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 text-sm font-semibold uppercase tracking-[0.3em] text-white/80">
        {variant === 'newsletter' ? <MailPlus className="h-4 w-4" aria-hidden /> : <PenLine className="h-4 w-4" aria-hidden />}
        {variant === 'newsletter' ? 'Newsletter' : 'Themenwunsch'}
      </div>
      <h3 className="mt-4 text-2xl font-semibold text-white">{title ?? defaultCopy[variant].title}</h3>
      ...
      <form onSubmit={handleSubmit} className="mt-5 space-y-4">
        ...
        <label className="space-y-1 text-sm" htmlFor={fieldId('message')}>
          <span className="font-semibold text-white/90">
            {variant === 'newsletter' ? 'Optional: Welche Updates interessieren dich?' : 'Was sollen wir behandeln?'}
          </span>
          <Textarea
            id={fieldId('message')}
            value={form.message}
            ...
            rows={4}
            placeholder={variant === 'newsletter' ? 'z. B. Produkt-Roadmap, Unternehmensangebote, Research ...' : 'z. B. „Mehr zu PHQ-9 Ergebnissen bei Jugendlichen“'}
            required
            className="border-white/30 bg-white/10 text-white placeholder:text-white/60"
          />
          <span className="text-xs text-white/70">Mindestens 10 Zeichen – damit wir zielgerichtet antworten können.</span>
        </label>
        ...
        <p className="text-[11px] text-white/70">
          Keine Spam-Mails. Wir melden uns nur, wenn neue Blogposts, Studien oder Produktfeatures live gehen.
        </p>
      </form>
    </div>
  )
