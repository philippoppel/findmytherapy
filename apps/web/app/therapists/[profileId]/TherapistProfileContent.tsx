...
export function TherapistProfileContent({ profile, profileId, fromTriage, structuredData }: Props) {
  const { t } = useTranslation();

  const formatCurrencyRange = (min?: number | null, max?: number | null) => {
    if (!min && !max) {
      return t('therapistProfile.onRequest');
    }
    if (min && max) {
      return `${min} € – ${max} €`;
    }
  ...
  const experience = profile.yearsExperience ? t('therapistProfile.yearsExperienceLabel', { years: profile.yearsExperience }) : undefined;
            ...
            <div className="flex items-center justify-between h-16">
              {fromTriage ? (
                <BackButton className="inline-flex items-center gap-2 text-slate-600 hover:text-primary-600 transition-colors group">
                  ...
                  <span>{t('therapistProfile.backToRecommendations')}</span>
                </BackButton>
              ) : (
                <Link
                  href="/therapists"
                ...
                >
                  <ArrowLeft className="h-5 w-5 transition-transform group-hover:-translate-x-0.5" aria-hidden />
                  <span>{t('therapistProfile.backToOverview')}</span>
                </Link>
              )}
            </div>
          </div>
                    ...
                    <div className="relative h-32 w-32 md:h-40 md:w-40 overflow-hidden rounded-2xl border-4 border-white/20 shadow-2xl ring-4 ring-white/10">
                      {profile.profileImageUrl ? (
                        <Image
                          ...
                          alt={`Portrait ${profile.displayName ?? t('therapistProfile.therapist')}`}
                          width={200}
                          height={200}
                    ...
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        <h1 className="text-4xl md:text-5xl font-bold text-white">
                          {profile.displayName ?? t('therapistProfile.therapist')}
                        </h1>
                        {profile.status === 'VERIFIED' && (
                      ...
                      <p className="text-lg md:text-xl text-primary-100 font-medium mb-3">
                        {profile.title ?? t('therapistProfile.therapist')}
                      </p>
                      {profile.headline && (
                    ...
                    <div className="flex flex-wrap gap-4 text-sm">
                      <div className="flex items-center gap-2 px-3 py-1.5 bg-white/10 backdrop-blur-sm rounded-lg">
                        ...
                        <span className="text-white/90">
                          {profile.online
                            ? `${profile.city ?? t('therapistProfile.online')} + ${t('therapistProfile.online')}`
                            : (profile.city ?? t('therapistProfile.onSite'))}
                        </span>
                      </div>
                      {experience && (
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-white/10 backdrop-blur-sm rounded-lg">
                          <Briefcase className="h-4 w-4 text-primary-200" />
                          <span className="text-white/90">{experience} {t('therapistProfile.experienceLabel')}</span>
                        </div>
                      )}
                      {profile.rating && (
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-white/10 backdrop-blur-sm rounded-lg">
                          ...
                          <span className="text-white/70">
                            ({profile.reviewCount ?? 0} {t('therapistProfile.reviews')})
                          </span>
                        </div>
                      )}
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  >
                    <Link href={contactHref} prefetch={false}>
                      ...
                      {t('therapistProfile.requestAppointment')}
                    </Link>
                  </Button>
                  {FEATURES.ASSESSMENT && (
                    <Button
                    ...
                    >
                      <Link href="/triage" prefetch={false}>
                        {t('therapistProfile.startAssessment')}
                      </Link>
                    </Button>
                  )}
                </div>
              ...
              <div className="lg:flex lg:items-start lg:justify-end">
                <div className="rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 p-6 shadow-2xl w-full lg:w-auto lg:min-w-[300px]">
                  <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
                    ...
                    {t('therapistProfile.appointmentRequest')}
                  </h3>
                  <div className="space-y-4">
                    <div className="text-sm text-white/90 space-y-2">
                      <div className="flex items-start gap-2">
                        <Clock className="h-4 w-4 mt-0.5 shrink-0 text-primary-200" />
                        <span>{profile.responseTime ?? t('therapistProfile.responseWithin48h')}</span>
                      </div>
                      <div className="flex items-start gap-2">
                        <Euro className="h-4 w-4 mt-0.5 shrink-0 text-primary-200" />
                        <span>
                          {formatCurrencyRange(profile.priceMin, profile.priceMax)} {t('therapistProfile.perUnit')}
                        </span>
                      </div>
                    </div>
                    <Button
                      asChild
                      className="w-full bg-white text-primary-700 hover:bg-primary-50 font-semibold"
                    >
                      <Link href={contactHref} prefetch={false}>
                        {t('therapistProfile.contactNowFull')}
                      </Link>
                    </Button>
                    <p className="text-xs text-white/70 text-center">{t('therapistProfile.firstSessionFree')}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-8">
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-3">
                    <h3 className="font-semibold text-slate-900 flex items-center gap-2">
                      ...
                      {t('therapistProfile.therapeuticApproach')}
                    </h3>
                    <p className="text-slate-600 text-sm leading-relaxed pl-10">
                      {profile.approachSummary ?? t('therapistProfile.infoFollows')}
                    </p>
                  </div>
                  <div className="space-y-3">
                    <h3 className="font-semibold text-slate-900 flex items-center gap-2">
                      ...
                      {t('therapistProfile.targetGroupsExperience')}
                    </h3>
                    <p className="text-slate-600 text-sm leading-relaxed pl-10">
                      {profile.experienceSummary ?? t('therapistProfile.infoFollows')}
                    </p>
                  </div>
                </div>
              </section>
              ...
              <section className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
                ...
                <div className="space-y-6">
                  ...
                  {(profile.modalities ?? []).length > 0 && (
                    <div>
                      <h3 className="font-semibold text-slate-900 mb-3 text-sm uppercase tracking-wide">
                        {t('therapistProfile.therapeuticModalities')}
                      </h3>
                      <div className="flex flex-wrap gap-2">
              ...
              {(profile.services ?? []).length > 0 && (
                <section className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
                  <h2 className="text-2xl font-bold text-slate-900 mb-6 flex items-center gap-3">
                    ...
                    {t('therapistProfile.offeredServices')}
                  </h2>
              ...
              {profile.courses.length > 0 && (
                <section className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
                  <h2 className="text-2xl font-bold text-slate-900 mb-6 flex items-center gap-3">
                    <BookOpen className="h-6 w-6 text-primary-600" />
                    {t('therapistProfile.programsCourses')}
                  </h2>
                  <div className="grid gap-6 md:grid-cols-2">
            ...
            <aside className="space-y-6 lg:sticky lg:top-24 lg:self-start">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
                  ...
                  {t('therapistProfile.qualifications')}
                </h3>
                <dl className="space-y-4 text-sm">
                  {profile.licenseAuthority && (
                    <div>
                      <dt className="font-medium text-slate-500 mb-1">{t('therapistProfile.licenseAuthority')}</dt>
                      <dd className="text-slate-900">{profile.licenseAuthority}</dd>
                    </div>
                  )}
                  {profile.licenseId && (
                    <div>
                      <dt className="font-medium text-slate-500 mb-1">{t('therapistProfile.licenseId')}</dt>
                      <dd className="text-slate-900 font-mono text-xs">{profile.licenseId}</dd>
                    </div>
                  )}
                  {profile.yearsExperience && (
                    <div>
                      <dt className="font-medium text-slate-500 mb-1">{t('therapistProfile.professionalExperience')}</dt>
                      <dd className="text-slate-900">{t('therapistProfile.yearsExperienceLabel', { years: profile.yearsExperience })}</dd>
                    </div>
                  )}
                  {(profile.languages ?? []).length > 0 && (
                    <div>
                      <dt className="font-medium text-slate-500 mb-1">{t('therapistProfile.languages')}</dt>
                      <dd className="text-slate-900">{(profile.languages ?? []).join(', ')}</dd>
                    </div>
                  )}
                </dl>
              </div>
                ...
                <dl className="space-y-4 text-sm">
                  ...
                  <div>
                    <dt className="font-medium text-slate-700 mb-1 flex items-center gap-2">
                      ...
                      {t('therapistProfile.responseTime')}
                    </dt>
                    <dd className="text-slate-900">
                      {profile.responseTime ?? t('therapistProfile.within48Hours')}
                    </dd>
                  </div>
                  {profile.availabilityNote && (
                    <div>
                      <dt className="font-medium text-slate-700 mb-1 flex items-center gap-2">
                        ...
                        {t('therapistProfile.availability')}
                      </dt>
              ...
              <div className="bg-gradient-to-br from-primary-600 to-primary-800 rounded-2xl p-6 text-white">
                <h3 className="font-bold text-lg mb-2">{t('therapistProfile.readyForFirstStep')}</h3>
                <p className="text-primary-100 text-sm mb-4">
                  {t('therapistProfile.contactForConsultation')}
                </p>
                <Button
                  asChild
                  className="w-full bg-white text-primary-700 hover:bg-primary-50 font-semibold"
