      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6" aria-label="Schnellaktionen und Termine">
            ...
            {profile?.appointments && profile.appointments.length > 0 ? (
              <ul className="space-y-3">
                {profile.appointments.map((appointment) => (
                  <li key={appointment.id} className="flex items-start justify-between gap-4 rounded-lg border border-divider p-3 hover:bg-neutral-50 transition-colors">
                ...
                {profile.courses.map((course) => (
                  <li key={course.id} className="py-3 flex justify-between items-center">
                    ...
                    <Link
                      ...
                      className="text-sm font-medium text-primary-900 hover:text-primary-950 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-800 rounded"
                    >
                      Öffnen
                    </Link>
                  </li>
        ...
        <aside className="space-y-6">
          ...
          <div className="bg-primary-50 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-primary-950 mb-2">Listing-Status</h3>
...
function EmptyState({ icon: Icon, title, description, actionHref, actionLabel }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-start gap-4 rounded-lg border border-dashed border-neutral-300 p-6">
      ...
      <a
        href={actionHref}
        className="inline-flex items-center text-sm font-medium text-primary-900 hover:text-primary-950 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-800 rounded"
      >
        {actionLabel}
      </a>
    </div>
