...
test.describe('Therapeut:innen Suche & Filter', () => {
  ...
  test('should load therapists page and display all verified therapists', async ({ page }) => {
    ...
    // Should show heading
    await expect(
      page.getByRole('heading', { name: /Finde die Therapeut.*in/i })
    ).toBeVisible()
  ...
  test('should show "no results" message with helpful actions', async ({ page }) => {
    ...
    const matchingLink = page.getByRole('link', { name: /Matching/i }).or(page.getByRole('button', { name: /Matching/i }))
  ...
  test('should maintain filter state when navigating back', async ({ page }) => {
    ...
    const stillFiltered = await page.getByText(/Thomas Wagner/i).isVisible().catch(() => false)
