...
async function main() {
  ...
  for (const therapist of therapistsWithoutCoords) {
    ...
    if (coords) {
      ...
      console.log(`✅ Updated ${therapist.displayName || therapist.id} (${therapist.city}): ${coords.lat}, ${coords.lng}`)
      updated++
    } else {
      console.log(`⚠️  Skipped ${therapist.displayName || therapist.id} - Unknown city: "${therapist.city}"`)
      skipped++
    }
  }
