...
async function testMatching() {
  try {
    ...
    if (result1.filtered && result1.filtered.length > 0) {
      ...
      const filterReasons = result1.filtered.reduce(
        (acc, f) => {
    ...
    result1.matches.forEach((match, i) => {
      console.log(
        `${i + 1}. ${match.therapist.displayName} - Score: ${(match.score * 100).toFixed(1)}%`,
      );
      console.log(
        `   📍 ${match.therapist.city || 'Online'}${match.distanceKm ? ` (${match.distanceKm} km)` : ''}`,
      );
      console.log(
        `   🎯 Spezialisierungen: ${match.therapist.specialties?.slice(0, 3).join(', ')}`,
      );
      console.log(
        `   💰 ${match.therapist.priceMin && match.therapist.priceMax ? `€${match.therapist.priceMin / 100}-${match.therapist.priceMax / 100}` : 'k.A.'}`,
      );
      console.log(`   Score-Details:`);
      console.log(
        `      - Spezialisierung: ${(match.scoreBreakdown.components.specialty.score * 100).toFixed(0)}%`,
      );
      console.log(
        `      - Distanz: ${(match.scoreBreakdown.components.distance.score * 100).toFixed(0)}%`,
      );
      console.log(
        `      - Verfügbarkeit: ${(match.scoreBreakdown.components.availability.score * 100).toFixed(0)}%`,
      );
      console.log(
        `      - Methoden: ${(match.scoreBreakdown.components.methods.score * 100).toFixed(0)}%`,
      );
    ...
    if (result5.filtered && result5.filtered.length > 0) {
      ...
      const filterReasons = result5.filtered.reduce(
        (acc, f) => {
          acc[f.reason] = (acc[f.reason] || 0) + 1;
          return acc;
        },
    ...
    result5.matches.forEach((match, i) => {
      ...
      console.log(`   📍 ${match.therapist.city || 'Online'}`);
      console.log(
        `   🎯 Spezialisierungen: ${match.therapist.specialties?.slice(0, 5).join(', ')}`,
      );
      ...
      console.log(
        `      - Sprache: ${(match.scoreBreakdown.components.language.score * 100).toFixed(0)}%`,
      );
      console.log('');
    });
