...
async function checkTherapistData() {
  try {
    ...
    console.log(`Public & Verified: `, await prisma.therapistProfile.count({
    ...
    const stats = {
      missingSpecialties: therapists.filter(t => !t.specialties || t.specialties.length === 0).length,
      missingLanguages: therapists.filter(t => !t.languages || t.languages.length === 0).length,
      missingModalities: therapists.filter(t => !t.modalities || t.modalities.length === 0).length,
      missingInsurance: therapists.filter(t => !t.acceptedInsurance || t.acceptedInsurance.length === 0).length,
