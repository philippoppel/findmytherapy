...
const QUALIFICATIONS_BY_SPECIALTY: Record<string, string[]> = {
  ...
  ADHS: ['Spezialisierung auf ADHS-Diagnostik und -Behandlung', 'Fortbildung in Neurofeedback'],
  Beziehungsprobleme: ['Zertifizierung in Paartherapie', 'Weiterbildung in Systemischer Therapie'],
};
    ...
    for (const therapist of therapists) {
      ...
      if (!therapist.acceptedInsurance || therapist.acceptedInsurance.length === 0) {
        const randomInsurance =
          INSURANCE_OPTIONS[Math.floor(Math.random() * INSURANCE_OPTIONS.length)];
      ...
      if (!therapist.ageGroups || therapist.ageGroups.length === 0) {
        const randomAgeGroups =
          AGE_GROUP_OPTIONS[Math.floor(Math.random() * AGE_GROUP_OPTIONS.length)];
      ...
      if (therapist.specialties && therapist.specialties.length > 0) {
        ...
        if (newQuals.length > 0) {
          ...
          needsUpdate = true;
          console.log(
            `  ✅ ${therapist.displayName}: ${newQuals.length} Qualifikationen hinzugefügt`,
          );
        }
      }
