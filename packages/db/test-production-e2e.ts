...
async function testMicrositePageLoad() {
  ...
  try {
    ...
    logTest('Microsite Page Load', 'FAILED', `Request failed: ${error.message}`, {
...
async function testDossierAPIStructure() {
  ...
  try {
    ...
    if (status === 401 || status === 403) {
      ...
      logTest(
        'Dossier GET Auth Guard',
        'WARNING',
        'Got 404 (endpoint exists, auth might be checked after)',
        {
          status,
        },
      );
...
async function testDatabaseSchema() {
  ...
  try {
    ...
    logTest(
      'TherapistMicrositeRedirect Table',
      'PASSED',
      `Table exists with ${redirectsCount} records`,
    );
...
async function testDatabaseTestData() {
  ...
  try {
    ...
    if (therapistProfile) {
      ...
      logTest(
        'Therapist Microsite Profile',
        'FAILED',
        `No published microsite found for slug: ${TEST_MICROSITE_SLUG}`,
      );
    }
