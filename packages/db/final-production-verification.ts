...
async function testMicrositeFeatures() {
  ...
  try {
    const testLead = {
      ...
      message:
        'This is an automated production E2E test. The lead form submission is working correctly.',
      consent: true,
      metadata: {
...
function generateReport() {
  ...
  categories.forEach((category) => {
    ...
    console.log(`\n${category}:`);
    console.log(
      `  ${categoryPassed}/${categoryTotal} passed (${((categoryPassed / categoryTotal) * 100).toFixed(0)}%)`,
    );
  ...
  console.log(
    '    TherapistMicrositeRedirect, SessionZeroDossier, DossierAccessLog, ClientConsent',
  );
