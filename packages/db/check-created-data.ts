...
async function checkCreatedData() {
  ...
  recentLeads.forEach((lead, index) => {
    ...
    console.log(`   Therapist: ${lead.therapistProfile.displayName} (${lead.therapistProfile.micrositeSlug})`)
  ...
  if (allLeads.length > 0) {
    allLeads.forEach((lead, index) => {
      console.log(`${index + 1}. ${lead.name} (${lead.email}) - ${lead.status} - ${lead.createdAt}`)
    })
  } else {
