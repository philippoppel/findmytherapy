...
const buildFallbackEnvInput = (sanitizedEnv: Record<string, string | undefined>, nodeEnv: string) => ({
  NODE_ENV: nodeEnv,
  DATABASE_URL:
...
export const env = (() => {
  try {
    ...
    const allowFallbackInProduction = toBoolean(sanitizedEnv.ALLOW_INCOMPLETE_ENV);

    if (nodeEnv !== 'production' || allowFallbackInProduction) {
      const reason =
        nodeEnv !== 'production'
          ? '[config] Environment not fully configured yet:'
          : '[config] ALLOW_INCOMPLETE_ENV=1 detected – using fallback environment values during production build:';
