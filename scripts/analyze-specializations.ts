...
function groupSimilarSpecialties(specialties: Map<string, SpecialtyStats>): Map<string, string[]> {
  ...
  const sortedSpecialties = Array.from(specialties.entries()).sort(
...
async function main() {
  ...
  console.log(
    `Average specialties per profile: ${(totalSpecialties / profiles.length).toFixed(1)}`,
  );
  console.log('');

  // Sort by frequency
  const sortedSpecialties = Array.from(specialtyMap.entries()).sort(
    (a, b) => b[1].count - a[1].count,
  );
  ...
  sortedSpecialties.slice(0, 30).forEach(([specialty, stats], index) => {
    console.log(
      `${(index + 1).toString().padStart(2)}. ${specialty.padEnd(40)} ${stats.count.toString().padStart(3)}x`,
    );
