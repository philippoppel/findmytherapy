...
async function main() {
  ...
  try {
    ...
    // Find submit button
    const submitBtn = await page.$('input[name="search_available"]') ||
    ...
    if (submitBtn) {
      ...
      const results = await page.evaluate(() => {
        ...
        return {
          ...
          profileLinks,
          resultCount,
          hasResults: bodyText.toLowerCase().includes('ergebnis') ||
        ...
        if (results.profileLinks[0]) {
          ...
          const profileData = await page.evaluate(() => {
            ...
            const data: any = {
              ...
              hasSocial: text.includes('LinkedIn') || text.includes('Instagram') || text.includes('Facebook'),
              hasPrice: text.includes('€') || text.includes('Euro') || text.includes('Preis'),
            }

            // Try to find specific elements
            const photoEl = document.querySelector('img.profile-photo, img.person-photo, img[alt*="Foto"]')
            if (photoEl) {
