...
async function main() {
  ...
  try {
    ...
    quickSearchFields.inputs.forEach((f) => {
      console.log(
        `  [${f.tag}] type="${f.type}" name="${f.name}" id="${f.id}" placeholder="${f.placeholder}"`,
      );
    });
    console.log('\nButtons:');
    ...
    if (nameField) {
      ...
      if (submitBtn) {
        ...
        const pageContent = await page.evaluate(() => {
          ...
          const hasResults =
            text.includes('Ergebnis') || text.includes('gefunden') || text.includes('Therapeut');
    ...
    const hasResultsViaUrl =
      directUrlContent.includes('Müller') || directUrlContent.includes('gefunden');
